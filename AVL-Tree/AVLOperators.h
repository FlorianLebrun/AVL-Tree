...
*
** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **/
template <class TNode, class TNodeHandler = AVLDefaultHandler<TNode>, typename TCompResult = int>
class AVLOperators {
...
public:
   ...
   struct IComparable {
      virtual TCompResult compare(Node node) = 0;
   };
   ...
   public:
      ...
      struct iterator {
         ...
         bool operator != (iterator_end& it) { return !!c.current; }
      };
      struct iterator_end {
   ...
   static Node removeAt(Node root, IComparable* comparable, Node& result)
   {
      ...
      TCompResult c = comparable->compare(root);
      if (c > 0) {
   ...
   static Node removeUpper(Node root, IComparable* comparable, Node& result)
   {
      struct internal {
         static Node removeUpper(Node root, IComparable* comparable, Node& result) {
            // Empty sub tree, nothing to remove
            ...
            TCompResult c = comparable->compare(root);
            if (c > 0) {
               At::right(root) = removeUpper(At::right(root), comparable, result);
               return rebalance(root);
            }
            else if (c < 0) {
               At::left(root) = removeUpper(At::left(root), comparable, result);
               if (result) return rebalance(root);
            }
      ...
      result = 0;
      return internal::removeUpper(root, comparable, result);
   }
   ...
   static Node insertAt(Node root, IInsertable* insertable, Node& result) {
      ...
      TCompResult c = insertable->compare(root);
      if (c < 0) {
